...
BEGIN_MESSAGE_MAP(CPropertiesBar, CDialogBar)
	...
	ON_BN_CLICKED(ID_EVENT_EMPTY, OnEventEmpty)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CPropertiesBar::Create(CWnd *pParentWnd,UINT nStyle)
{
	...
	if(m_EventLib.LoadEvents(m_hWnd))
	{
		RECT rectEventPlace;
...
void CPropertiesBar::OnSelChangeEvents() 
{
	...
	if(iCurSel!=CB_ERR && iCurSel != m_iCurSel)
	{
		...
		m_EventLib.SetDefEventLib(iCurSel);
		m_EventLib.Show(TRUE);
		m_iCurSel=iCurSel;
	}
}

void CPropertiesBar::OnEventEmpty() 
{
	if(m_iCurSel != CB_ERR)
		m_EventLib.Empty();
}
