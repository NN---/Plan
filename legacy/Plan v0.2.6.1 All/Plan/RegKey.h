...
public:
	...
	// Create
	LONG Create(HKEY hKey, LPCTSTR lpSubKey = NULL, LPTSTR lpClass = REG_NONE,
	...
	// Open
	LONG Open(HKEY hKey, LPCTSTR lpSubKey = NULL, REGSAM samDesired = KEY_ALL_ACCESS);
	// Connect
...
// Create
inline LONG RegKey::Create(HKEY hKey, LPCTSTR lpSubKey /*=NULL*/, LPTSTR lpClass, 
...
// Open
inline LONG RegKey::Open(HKEY hKey, LPCTSTR lpSubKey /*=NULL*/, REGSAM samDesired)
{
	HKEY hkey;
...
{
	ATLASSERT(m_hKey != NULL);
	ATLASSERT(lpNewFile != NULL);
	ATLASSERT(lpOldFile != NULL);
...
inline LONG RegKey::QueryValue(DWORD &dwValue, LPCTSTR lpValueName)
{
	...
	ATLASSERT(lRes != ERROR_SUCCESS || dwType != REG_DWORD || dwCount == sizeof(DWORD));
	return lRes;
}
...
inline LONG RegKey::QueryValue(LPTSTR lpValue, LPCTSTR lpValueName,	LPDWORD lpCount)
{
	...
	ATLASSERT(lRes!=ERROR_SUCCESS || (dwType != REG_SZ || dwType != REG_MULTI_SZ ||
