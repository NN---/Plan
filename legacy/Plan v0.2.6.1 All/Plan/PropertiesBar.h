...
public:
	...
	BEGIN_MSG_MAP(thisClass)
		...
		// Commands
		COMMAND_ID_HANDLER(IDC_ONCE, OnOnceDaily)
		COMMAND_ID_HANDLER(IDC_DAILY, OnOnceDaily)
	...
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		/*
		BOOL bHandled;
		OnOnceDaily(BN_CLICKED,IDC_ONCE,GetDlgItem(IDC_ONCE),bHandled);
		*/
		OnOnceDaily(BN_CLICKED,IDC_ONCE,GetDlgItem(IDC_ONCE),(BOOL &)(*(BOOL *)NULL));
		return 0;
	}
	...
	// WM_COMMAND - IDC_ONCE
	LRESULT OnOnceDaily(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		bool bOnce = wID==IDC_ONCE;
		::EnableWindow(GetDlgItem(IDC_DAYS),bOnce?FALSE:TRUE);
		::EnableWindow(GetDlgItem(IDC_DATE),bOnce?TRUE:FALSE);
	...
	void FillData()
	{
		...
		unsigned uDay;
		CString str;
		for(uDay=IDS_SUNDAY;uDay<=IDS_FRIDAY;uDay++)
		{
			str.LoadString(uDay);
				...
				PCEventID pcEventID, const LPBYTE pData)
	{
		...
		cmbEvents.SetCurSel(_EventLib.Find(pcEventID,TRUE));

	//	SendMessage(WM_COMMAND,MAKELONG(CBN_SELCHANGE,IDC_EVENTS));
		BOOL bHandled;
		OnSelChangeEvents(CBN_SELCHANGE,IDC_EVENTS,GetDlgItem(IDC_EVENTS),bHandled);
