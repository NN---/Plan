...
{
	EventLib &rEventLib=((CMainFrame *)AfxGetMainWnd())->GetPropertiesBar().GetEventLib();
	RegKey regEvents;
	DWORD dwEvents;
	TCHAR szEvent[5];

	aID.Add(rEventLib.GetEventID());
	dwEvents=aID.GetUpperBound();
	
	regEvents.Create(HKEY_CURRENT_USER,_T("Software\\NN\\Plan\\Events"));
	wsprintf(szEvent,_T("%d"),dwEvents);
	regEvents.Create(regEvents,szEvent);

	rEventLib.Write(regEvents);
}
