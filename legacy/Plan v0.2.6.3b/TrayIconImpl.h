...
template <class T>
class ATL_NO_VTABLE CTrayIconImpl
{
private:
	UINT WM_TRAYICON;
	...
	int m_nMenu;
public:	
	CTrayIconImpl() : m_bInstalled(false), m_nDefault((UINT)-1)
	{
	...
	bool InstallIcon(LPCTSTR lpszToolTip, HICON hIcon, UINT nID, int nMenu=0)
	{
	...
	bool RemoveIcon()
	{
		...
		// Remove
		m_nid.uFlags = 0;
		bRes = Shell_NotifyIcon(NIM_DELETE, &m_nid) ? true : false;
		if(bRes)
			m_bInstalled = 0;
		return bRes;
	}
	...
	LRESULT OnTrayIcon(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		if (LOWORD(lParam) == WM_RBUTTONUP)
		{
			...
			if (!oMenu.LoadMenu(m_nid.uID))
				return 0;
			// Get the sub-menu
			CMenuHandle oPopup(oMenu.GetSubMenu(m_nMenu));
			// Prepare
