...
BOOL CPropertiesBar::Create(CWnd *pParentWnd,UINT nStyle)
{
	...
	pApp->m_EventLib.LoadEvents(m_hWnd);
	RECT rectEventPlace;
	GetDlgItem(IDC_EVENTPLACE)->GetWindowRect(&rectEventPlace);
	ScreenToClient(&rectEventPlace);

	unsigned i;
	unsigned nEvents=pApp->m_EventLib.GetEventCount();
	for(i=0;i<nEvents;i++)
	{
		pApp->m_EventLib.SetEventLib(i);
		m_cmbEvents.AddString(pApp->m_EventLib.GetEventName());
		pApp->m_EventLib.Move(rectEventPlace,FALSE);
	}
...
void CPropertiesBar::OnSelChangeEvents() 
{
	...
	{
		CPlanApp *pApp = (CPlanApp *)AfxGetApp();
		pApp->m_EventLib.Show(FALSE);
		pApp->m_EventLib.SetEventLib(iCurSel);
		pApp->m_EventLib.Show(TRUE);
		m_iCurSel=iCurSel;
	}
}
...
{
	if(m_iCurSel!=CB_ERR)
		((CPlanApp *)AfxGetApp())->m_EventLib.Empty();
}
