...
void CPlanView::SetColumns()
{
	...
	CPlanDoc *lpDoc=GetDocument();
	int i,iItems=pHeaderCtrl->GetItemCount();
	int nWidth=0;
	TCHAR str[20];
	CSize sizenew;
	LVCOLUMN lvcolumn;
	...
	lvcolumn.cchTextMax=lstrlen(str);
	lvcolumn.cx=lpDoc->ColumnWidth[SUB_ITEM_EVENT];
	ListCtrl.SetColumn(SUB_ITEM_EVENT,&lvcolumn);
	...
	for(i=1;i<NUM_SUB_ITEMS;i++)
	{
		...
		if(lpDoc->ColumnWidth[i])
			lvcolumn.cx=lpDoc->ColumnWidth[i];
		else
		{
			SYSTEMTIME st;
			GetSystemTime(&st);
			...
			lvcolumn.cx=pDC->GetTextExtent(str,lstrlen(str)).cx+5;
			lpDoc->ColumnWidth[i]=(WORD)lvcolumn.cx;
		}
		...
		ListCtrl.SetColumn(SUB_ITEM(i),&lvcolumn);
	...
	ListCtrl.SetColumnOrderArray(NUM_SUB_ITEMS,array);
}
...
BOOL CPlanView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	if(pNMHDR->code==HDN_ENDTRACKA||
		pNMHDR->code==HDN_ENDTRACKW)
	{
		HD_NOTIFY *phdn=(HD_NOTIFY *)pNMHDR;
		GetDocument()->ColumnWidth[phdn->iItem]=(WORD)phdn->pitem->cxy;
	}
	else
	if(pNMHDR->code==HDN_ENDDRAG)
	{
		...
		GetListCtrl().GetColumnOrderArray(array,NUM_SUB_ITEMS);

		for(i=0;i<NUM_SUB_ITEMS;i++)
			*(lpDoc->ColumnOrder+i)=(BYTE)*(array+i);
	}
	else
