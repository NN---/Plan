...
#define NF(fname) Event##fname
	...
	EVENT_IMPORTEXPORT BOOL NF(Get)(LPBYTE pData);
	EVENT_IMPORTEXPORT void NF(Set)(const LPBYTE pData);
	EVENT_IMPORTEXPORT UINT NF(Size)(void);
	...
	#define DEFPF2(type,fname,t1,t2) \
		...
		DEFPF1(BOOL,Get,LPBYTE);
		DEFPF1(void,Set,const LPBYTE);
		DEFPF0(UINT,Size);
...
public:
	...
	BOOL Do();
	//
	BOOL Get(LPBYTE pData);
	void Set(const LPBYTE pData);
	UINT Size(void);
};
...
inline BOOL EventLib::Get(LPBYTE pData)
{
	ASSERT(uEventLib<=nEventLib);
	RDGPA(*(phEventLib+uEventLib),Get)(pData);
}

inline void EventLib::Set(const LPBYTE pData)
{
	ASSERT(uEventLib<=nEventLib);
	FDGPA(*(phEventLib+uEventLib),Set)(pData);
}

inline UINT EventLib::Size()
{
	ASSERT(uEventLib<=nEventLib);
	RDGPA(*(phEventLib+uEventLib),Size)();
}
