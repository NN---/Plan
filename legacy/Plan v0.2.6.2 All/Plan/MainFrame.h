...
class /*ATL_NO_VTABLE*/ CMainFrame : public DocumentFrame<CMainFrame>,
...
{					
	...
	typedef DocumentFrame<CMainFrame> baseClass;
public:
	bool m_bCreateSuccess;
	CMainFrame() { m_bCreateSuccess=false;}
	...
	*/
	...
	LRESULT OnToolsOptions(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		OptionsSheet.DoModal(&m_Options);	
		return 0;
	}
	...
	void SaveRegistry()
	{
		...
		if(lRes==ERROR_SUCCESS)
		{
			...
			if(m_Options.bLoadOnStartup)
			{
				...
				lstrcat(szFileName,__argv[0]/*__targv[0]*/);
				lstrcat(szFileName,_T(" -tray\"")); // Tray
	...
	void UpdateAllViews(ViewMessage::Types Type,int iEvent=0)
	{
