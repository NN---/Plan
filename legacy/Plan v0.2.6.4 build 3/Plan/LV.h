...
typedef LVTraitsImpl<WS_CHILD | WS_VISIBLE | LVS_REPORT | 
	LVS_SHOWSELALWAYS | LVS_OWNERDRAWFIXED | LVS_SINGLESEL,
	WS_EX_CLIENTEDGE,
		...
		void UpdateItemString()
		{
			...
			switch(m_wVirtualKeyCode)
			{
				...
				case VK_END:
					...
					break;
				case VK_PRIOR:
					m_strItem+=_T("Page Up");
					break;
				case VK_NEXT:
					m_strItem+=_T("Page Down");
					break;
				case VK_TAB:
					m_strItem+=_T("Tab");
					break;
...
public:
	...
	public:
		...
		void SetTimeFormat(const WTL::CString &rString)
		{
			m_strTimeFormat=rString;
			FormatTime();	
		}
		const Data &operator=(const Data &rData)
		{
